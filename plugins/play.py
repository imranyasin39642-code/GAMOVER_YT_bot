...
def cmd(cmds):
    """Custom command filter that matches /cmd and /cmd@botname."""
...
def register(app: Client):
    ...
    async def play_command(client: Client, message: Message):
        ...
        print(f"[Command] /vd in group {chat_id} | Query: '{query}'")
        if not query:
    ...
    async def audio_command(client: Client, message: Message):
        ...
        parts = (message.text or message.caption or "").split(maxsplit=1)
        query = parts[1].strip() if len(parts) > 1 else ""
        print(f"[Command] /ad in group {chat_id} | Query: '{query}'")
        if not query:
            await message.reply_text(make_card(
                f"{ROYAL_HEADER}❌ <b>Please specify a track name or YouTube URL!</b>\n\n"
                f"👉 <b>Click to Copy Examples:</b>\n"
    ...
    async def playlist_command(client: Client, message: Message):
        ...
        parts = (message.text or message.caption or "").split(maxsplit=1)
        query = parts[1].strip() if len(parts) > 1 else ""
        print(f"[Command] /list in group {chat_id} | Query: '{query}'")
        if query:
            ...
            asyncio.create_task(player_manager.play(
                chat_id=chat_id, youtube_url=query, mode="video",
                status_msg=status_msg, requested_by=req_name, requested_by_id=req_id
            ))
            return
    ...
    async def pl_resume_command(client: Client, message: Message):
        ...
        if not state or not state.get("playlist_id"):
            await message.reply_text(make_card(
                f"{ROYAL_HEADER}⚠️ <b>Iss group me koi saved playlist state nahi mili!</b>\nPehle koi playlist play karein."
            ))
            return
        ...
        if chat_id not in player_manager.pending_playlists:
            ...
            if entries:
                player_manager.pending_playlists[chat_id] = {
                    "url": playlist_url, "pl_id": pl_id, "mode": mode,
                    "entries": entries, "requested_by": req_name, "requested_by_id": req_id
                }
        if chat_id in player_manager.pending_playlists:
            asyncio.create_task(player_manager.execute_playlist(
                chat_id=chat_id, start_from_index=last_idx, mode=mode, status_msg=status_msg
            ))
        else:
            try:
    ...
    async def group_playlists_history_command(client: Client, message: Message):
        ...
        for idx, st in enumerate(states, start=1):
            ...
            buttons.append([
                InlineKeyboardButton(f"▶ Resume #{idx}: Song #{song_num}", callback_data=f"pl_hist_res|{chat_id}|{pl_id}|{st['mode']}|{last_idx}", style="success"),
                InlineKeyboardButton("🔄 Start Over", callback_data=f"pl_hist_res|{chat_id}|{pl_id}|{st['mode']}|0", style="primary")
            ])
        ...
        await send_styled(chat_id=chat_id, text=card_text, markup=InlineKeyboardMarkup(buttons))
    ...
    async def pl_history_resume_callback(client: Client, query: CallbackQuery):
        ...
        entries = await extract_youtube_playlist(playlist_url)
        if not entries:
            await edit_styled(chat_id=chat_id, text=f"{ROYAL_HEADER}❌ <b>Playlist load nahi ho saki ya private hai!</b>", message_id=query.message.id)
            return
        player_manager.pending_playlists[chat_id] = {
            "url": playlist_url, "pl_id": pl_id, "mode": mode,
            "entries": entries, "requested_by": req_name, "requested_by_id": req_id
        ...
        asyncio.create_task(player_manager.execute_playlist(
            chat_id=chat_id, start_from_index=start_idx, mode=mode, status_msg=query.message
        ))
        ...
        if args or cmd_name in ("seek", "cseek"):
            ...
            def _parse_time(time_str: str) -> int:
                ...
                if time_str.isdigit():
                    return int(time_str)
                total = 0
                for num_s, unit in re.findall(r'(\d+)\s*([smh])?', time_str):
                    n = int(num_s)
                    if unit == 'm':
                        total += n * 60
                    elif unit == 'h':
            ...
            if ok:
                from bot import send_styled
                await send_styled(chat_id=chat_id, text=f"⏩ <b>Stream seeked forward by {secs}s by:</b> {user_link}")
            else:
    ...
    async def stop_command(client: Client, message: Message):
        ...
        await message.reply_text(make_card(f"{ROYAL_HEADER}⏹ <b>ᴘʟᴀʏʙᴀᴄᴋ sᴛᴏᴘᴘᴇᴅ ʙʏ:</b> {user_link}"))

    # ── Owner-Only Approved Control Commands ────────────────────────────────
    @app.on_message(cmd(["approvecontrol", "approvedcontrol", "approve", "approved"]))
    async def approve_control_command(client: Client, message: Message):
        ...
        if message.reply_to_message and message.reply_to_message.from_user:
            ...
            if len(parts) > 1:
                try:
                    target_user = await client.get_users(parts[1].strip())
                except Exception:
                    pass
        if not target_user:
            await message.reply_text(make_card(
                f"{ROYAL_HEADER}❌ <b>Target User specify karein!</b>\n"
                f"Kisi user ke message par <b>reply</b> karke <code>/approve</code> chalayein ya <code>/approve @username</code> dein."
            ))
            return
        chat_id = message.chat.id
        ...
        await message.reply_text(make_card(
            ...
            f"⚡ <i>Ab aap is group me /pause, /resume, /skip, /stop control kar sakte hain!</i>"
        ))

    @app.on_message(cmd(["unapprovecontrol", "unapprovedcontrol", "unapprove", "unapproved"]))
    async def unapprove_control_command(client: Client, message: Message):
        ...
        if not target_user:
            await message.reply_text(make_card(
                f"{ROYAL_HEADER}❌ <b>Target User specify karein!</b>\n"
                f"Kisi user ke message par <b>reply</b> karke <code>/unapprove</code> chalayein."
            ))
            return
        chat_id = message.chat.id
    ...
    async def reload_system_command(client: Client, message: Message):
        ...
        await message.reply_text(make_card(
            ...
            f"⚡ <i>Stopping active streams & refreshing process... Please wait a few seconds!</i>"
        ))
        try:
    ...
    async def queue_command(client: Client, message: Message):
        ...
        if queued_songs:
            ...
            text += f"<b>Upcoming Queue:</b>\n<blockquote expandable>{chr(10).join(q_lines)}</blockquote>"
        else:
    ...
    async def help_command(client: Client, message: Message):
        help_text = f"Click the buttons below to get information about my commands.\n\n<i>Note: All commands can be used with /</i>"
        ...
        if action in ("play_stop", "play_skip", "play_pause", "play_resume", "play_loop", "play_delete"):
            ...
            if action == "play_stop":
                ...
                try:
                    ...
                    pass
                await send_styled(chat_id=chat_id, text=f"⏹ <b>Stream stopped by:</b> {user_link}")
                await query.answer("⏹ Stream Stopped!")
            elif action == "play_skip":
                await query.answer("⏭ Skipping track...")
                await player_manager.skip(chat_id)
                await send_styled(chat_id=chat_id, text=f"⏭ <b>Track skipped by:</b> {user_link}")
            elif action == "play_pause":
                ok = await player_manager.pause(chat_id)
                if ok:
                    await send_styled(chat_id=chat_id, text=f"⏸ <b>Stream paused by:</b> {user_link}")
                ...
                ok = await player_manager.resume(chat_id)
                if ok:
                    await send_styled(chat_id=chat_id, text=f"▶ <b>Stream resumed by:</b> {user_link}")
                ...
                await send_styled(chat_id=chat_id, text=f"🔁 <b>Loop toggled by:</b> {user_link}")
    ...
    async def playlist_resume_callbacks(client: Client, query: CallbackQuery):
        ...
        if action == "pl_do_start":
            ...
            try:
                ...
                pass
            asyncio.create_task(player_manager.execute_playlist(chat_id=chat_id, start_from_index=start_idx, mode=mode, status_msg=query.message))
            ...
            try:
                ...
                pass
            asyncio.create_task(player_manager.execute_playlist(chat_id=chat_id, start_from_index=0, mode=mode, status_msg=query.message))
